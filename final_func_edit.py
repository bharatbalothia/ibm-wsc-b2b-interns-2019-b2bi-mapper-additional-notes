    ...
    if inp_op==0:       # that is for input side rules
        ...
        if temp_id in globalVariables.dict_op_token:
            if temp_id in globalVariables.dict_opposite_name_id:
                ...
                globalVariables.extended_notes_set.add(field_id)
                        # to track that there should be a rule populated(used at the end)            output_note=final_note_for_field(field_ptr,temp_id,1,globalVariables)
            if output_note:
                if output_note[0]=='#':
