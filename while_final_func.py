...
# similar as field_func -- this one is to processs input side fields when i/p format is xml
def xml_field_func(field_ptr,grp_name,field_ct,fo,xml_tag,rec_id,globalVariables):
    ...
    ## populating the corresponding dictionaries
...
def xml_record_func(rec_ptr,grp_name,field_ct,fo,xml_tag,globalVariables): # field_ct-index number of the field in that segment, xml_tag-tag_name of segment
    rec_id = rec_ptr[0].text                                               # fo- file ptr where the rule would be written
...
## similar to the group_func of edi
...
# to process field on input side
def field_func(field_ptr,grp_name,seg_name,field_ct,field_tag,fo,globalVariables):
    ...
    for children in field_ptr:
        if children.tag.split('}')[1]=="ExplicitRule":
            if children.text!=None:
                temp_list=children.text.split(';')  # one field can have multiple rules written on it
                for item in temp_list:
                    globalVariables.ip_count        # count of input side rule number
                    globalVariables.ip_count=globalVariables.ip_count+1
                    globalVariables.dict_field[globalVariables.ip_count]=field_ptr[1].text      # save which field has which rule
                fo.write(children.text)
                fo.write('\n')
        ...
        if children.tag.split('}')[1]=='ImplicitRuleDef':
            if children[0].tag.split('}')[1]=='UseConstant':
                ...
                if children[0][1].text!='-1':
                    cond="If " + globalVariables.dict_1[children[0][1].text][2] + " exists "        # to store the condition
            ...
            if children[0].tag.split('}')[1]=="UseCode":
                ...
                code_note=''
                if children[0][2]=='yes':       # check the mxl file to get the idea of the hierarchy of tags
                    code_note+="Raise a compliance error if the code is not found in the list "
                if children[0][1].text in globalVariables.dict_1:
                    code_note += "Select codelist value from Table "+ children[0][0].text + " where field value = " +  field_ptr[1].text + " and assign it to field " +  globalVariables.dict_1[children[0][1].text][2]
                    # globalVariables.dict_notes[field_ptr[1].text] = code_note
                    print(globalVariables.dict_1[children[0][1].text][2])
                    # exit()
                    globalVariables.dict_token[globalVariables.dict_1[children[0][1].text][2]]=[[[globalVariables.dict_1[children[0][1].text][2]],'',code_note]]
                # else:
            ...
            if children[0].tag.split('}')[1]=='UseSelect':
                fieldfrom=field_ptr[0].text
                tablename_temp=children[0][0].text.split()
                tablename=' '.join(tablename_temp[-2:])      # to get keywords like Receiver and Sender
                subtable=children[0][1].text
                Mapfrom=children[0][3][0].text
                fieldto=children[0][3][1].text              # field where the rule is to be written
                ...
                if fieldto in globalVariables.dict_1:
                    select_stmt="Populate "+v[2]+" from CODELIST equal " + children[0][0].text +" into "+globalVariables.dict_1[fieldto][2]+" where sendercode equals "+field_ptr[1].text +'\n'
                    dict_ip_select[fieldto]=[tablename,subtable,Mapfrom,fieldfrom]
                    globalVariables.dict_token[globalVariables.dict_1[fieldto][2]]=[[globalVariables.dict_1[fieldto][2],'',select_stmt]]
                else:
                    ...
                    dict_ip_select[fieldto]=[tablename,subtable,Mapfrom,fieldfrom]
                    globalVariables.dict_token[field_ptr[1].text]=[[globalVariables.dict_1[fieldto][2],'',select_stmt]]
...
# to process segments in input side
def seg_func(seg_ptr,grp_name,fo,comp_count,flag,globalVariables):
    # from constants import count
    i=0
    field_ct=1      # keep count of the field number in each segment
    field_tag=''
    for children in seg_ptr:
        if children.tag.split('}')[1]=="ExplicitRule":
            for categ in children:
                if categ.text!=None:
                    ...
                    fo.write(categ.text)        # writing the explicit rule in input side in the textfile
                    fo.write('\n')
        if children.tag.split('}')[1]=='BlockSig':
            field_tag=children[0].text      # storing the tag name,.....would be used at the end to replace segName with TagName
        ...
        if children.tag.split('}')[1]=="Composite":
            ...
            # print(children.tag)
            if flag==0:
                field_func(children,grp_name,seg_ptr[1].text,field_ct,field_tag,fo,globalVariables)     # (passing field_ptr, group_name, segment_name, field_no., seg_tag, pointer of txt.file)
            else:
                # field_func(children,grp_name,seg_ptr[1].text,field_ct,field_tag,fo,globalVariables)
                field_func(children,grp_name,'Composite_'+grp_name+'_'+str(comp_count),field_ct,field_tag,fo,globalVariables)
            field_ct=field_ct+1

# to process input side group(edi/csv/idoc)
def group_func(group_ptr,fo,globalVariables):
    ...
    for children in group_ptr:
        if children.tag.split('}')[1]=="ExplicitRule":
            for categ in children:
                if categ.text!=None:
                    temp_list=categ.text.split(';')
                    for item in temp_list:
                        globalVariables.ip_count    # to keep count of the rule number
                        globalVariables.ip_count=globalVariables.ip_count+1
                        globalVariables.dict_field[globalVariables.ip_count]=group_ptr[1].text
                    fo.write(categ.text)        # writing input side explicit rules into the text file
                    fo.write('\n')
        ...
        elif children.tag.split('}')[1]=="Segment": # group_ptr[1].text  is the group name
            # print(children.tag)
            comp_count=0
...
# to process each field of the output side (if xml)
def xml_output_field_func(field_ptr,grp_name,field_op_ct,op_file,group_ptr,globalVariables):
    ...
    globalVariables.dict_notes[field_ptr[1].text] = ['', '', '', '', '',''] # initialize the dict which would have the final notes
    globalVariables.dict_useless[field_ptr[0].text] = field_ptr[1].text
    ...
    for children in field_ptr:
        if children.tag.split('}')[1]=="Link":
            ...
            if temp_ip in globalVariables.dict_1:
                globalVariables.link_notes_set.add(temp_id)     # put the field id in set to check at the end if note was genterated or not
                link_list=globalVariables.dict_1[temp_ip]
                if(len(link_list)==3 and link_list[0].split('_')[0].lower()!='temp'     # to check if it is a direct link or not
        ...
        if children.tag.split('}')[1] == "ExplicitRule":
            if children.text != None:
                ...
                temp_list = children.text.split(';')
                for item in temp_list:
                    globalVariables.op_count        # keep count of the number of the rule
                    globalVariables.op_count = globalVariables.op_count + 1
                    globalVariables.dict_op_field[globalVariables.op_count] = field_ptr[1].text     # save in dict which rule at which field
                op_file.write(children.text)
                op_file.write('\n')

        if children.tag.split('}')[1]=="StoreLimit":
            for tags in children:
                if tags.tag.split('}')[1]=="DataType":      # get the data_type of the field
                    datatype=tags.text
            if datatype=='numeric':
                datatype='integer'
        ...
        if children.tag.split('}')[1]=='ImplicitRuleDef':
            if children[0].tag.split('}')[1]=='UseConstant':
                globalVariables.standard_notes_set.add(temp_id)     # set to store fields which should have standard notes populated
                cond=''
                print(field_ptr[1].text)
                if children[0][1].text!='-1':       # to check for the condition in constant rules
                    cond=children[0][1].text
                globalVariables.dict_op_ct[field_ptr[0].text]=[children[0][0].text,cond]    # child[0][0].text- value that needs to be hardcoded
            acc_note = ''
            acc_flag = 0
            ...
            if children[0].tag.split('}')[1]=="UseCode":
                ...
                if children[0][2]=='yes':
                    code_note+="Raise a compliance error if the code is not found in the list "
                code_note = "Select codelist value from Table "+ children[0][0].text + " where field value = " +  field_ptr[1].text + " and assign it to field " +  children[0][1].text
                globalVariables.dict_notes[field_ptr[1].text][1] = code_note
            if children[0].tag.split('}')[1]=='UseSelect':  # for select statements
                fieldfrom=field_ptr[0].text
                tablename_temp = children[0][0].text.split()
                tablename = ' '.join(tablename_temp[-2:])
                subtable=children[0][1].text
                Mapfrom=children[0][3][0].text
                fieldto=children[0][3][1].text      # fieldto- field where the rule is to be written
                globalVariables.dict_op_select[fieldto]=[tablename,subtable,Mapfrom,fieldfrom]
                globalVariables.standard_notes_set.add(fieldto)     # set to store fields which should have standard notes populated
            if children[0].tag.split('}')[1] == 'UseSystemVariable':    # for DateTime standard rule
                datatype = ''
                format = ''
    ...
    field_ct_str = str(field_op_ct)     # get the number of the field in that segment
    if len(field_ct_str) == 1:
        field_ct_str = '0' + field_ct_str   # standardise it to make two digit number
    l=[grp_name,field_ct_str]
    globalVariables.dict_ind_field[field_ptr[1].text] = l
    globalVariables.dict_opposite_out[''.join(l)] = field_ptr[1].text
    name_field=field_ptr[1].text    # saving information in dictionaries
...
# process on record- one record has only one field
...
# to process particles (o/p side is xml)
def xml_output_particle_func(particle_ptr,grp_name,op_file,globalVariables):
    field_op_ct=1       # to send as argument to store the field number in each segment
    for children in particle_ptr:
        if children.tag.split('}')[1]=="ExplicitRule":
            for categ in children:
                if categ.text!=None:
                    temp_list=categ.text.split(';')
                    for item in temp_list:
                        globalVariables.op_count    # keep count of the rule number
                        globalVariables.op_count=globalVariables.op_count+1
                        globalVariables.dict_op_field[globalVariables.op_count]=particle_ptr[1].text    # save which rule comes on which field
...
# to process groups if output side is xml format
def xml_output_group_func(group_ptr,grp_name,field_ct,op_file,globalVariables):
    if globalVariables.out_grp_name == '':
        globalVariables.out_grp_name = group_ptr[1].text        # save the head group name...would be done only once
    for children in group_ptr:
        if children.tag.split('}')[1]=="ExplicitRule":  #process output side explicit rules
            for categ in children:
                if categ.text!=None:
                    temp_list=categ.text.split(';')
                    for item in temp_list:
                        globalVariables.op_count    # keep count of the rule number
                        globalVariables.op_count=globalVariables.op_count+1
                        globalVariables.dict_op_field[globalVariables.op_count]=group_ptr[1].text   # save which rule comes on which field
                    op_file.write(categ.text)
                    op_file.write('\n')
        if children.tag.split('}')[1]=="XMLParticleGroup":  # process particles
        ...
        if children.tag.split('}')[1]=="XMLRecord": # process records
            name=children[1].text
            ct_loop=children[7].text
            globalVariables.dict_seg_loop[name]=ct_loop
            xml_output_record_func(children,grp_name,field_ct,op_file,group_ptr,globalVariables)
        if children.tag.split('}')[1]=="XMLElementGroup":   # process groups
            xml_output_group_func(children,group_ptr[1].text,field_ct,op_file,globalVariables)

# to process field of output side edi/idoc/csv formats
def output_field(group_name,field_ptr,seg_name,field_tag,field_op_ct,op_file,name,ct_loop,globalVariables):
    # field_ptr[5].text = ''
    if field_ptr[3].text=='0':
        return
    datatype=''
    temp_id = field_ptr[0].text # id of the field
    temp_des = field_ptr[2].text    # description of field
    globalVariables.dict_notes[field_ptr[1].text]=['','','','','','']    # initializing the dict_notes to put the final notes later
    globalVariables.dict_useless[field_ptr[0].text]=field_ptr[1].text
    if field_ptr[1].text in globalVariables.dict_opposite_name_id:
    ...
    for child in field_ptr:
        if child.tag.split('}')[1]=="Link":
            temp_ip=child.text   # id of the field linked
            if temp_ip in globalVariables.dict_1:   # dict has id as key, and value - (seg_name, field_name)
                globalVariables.link_notes_set.add(temp_id)     # set to check in the end if all fields having links in input sides have generated notes(making log file)
                link_list=globalVariables.dict_1[temp_ip]
                print("DS2")
                print(link_list)
                if(len(link_list)==3 and link_list[0].split('_')[0].lower()!='temp'
                and link_list[1].split('_')[0].lower()!='temp' and link_list[2].split('_')[0].lower()!='temp'): # checking the presence of direct links
        ...
        if child.tag.split('}')[1]=="ExplicitRule": # checking for explicit rules in output side
            if child.text!=None:
                temp_list=child.text.split(';')
                for item in temp_list:
                    globalVariables.op_count    # counter to keep count of the rule number
                    globalVariables.op_count=globalVariables.op_count+1
                    globalVariables.dict_op_field[globalVariables.op_count]=field_ptr[1].text   # dict to record which rule comes at which field in o/p sied
                op_file.write(child.text)
                op_file.write('\n')

        if child.tag.split('}')[1]=="StoreLimit":
            for tags in child:
                if tags.tag.split('}')[1]=="DataType":  # saving dataType of each field
                    datatype=tags.text
            if datatype=='numeric':
                datatype='integer'


        if child.tag.split('}')[1]=='ImplicitRuleDef':  # to operate on standard rules in the output side
            if child[0].tag.split('}')[1]=='UseConstant':
                globalVariables.standard_notes_set.add(temp_id) # set to store fields which should have standard notes populated
                cond=''
                print(field_ptr[1].text)
                if child[0][1].text!='-1':  # to check for the condition in constant rules
                    cond=child[0][1].text
                globalVariables.dict_op_ct[field_ptr[0].text]=[child[0][0].text,cond]   # child[0][0].text- value that needs to be hardcoded
            acc_note = ''
            acc_flag = 0
            ...
            if child[0].tag.split('}')[1]=="UseCode":
                ...
                globalVariables.dict_notes[field_ptr[1].text][1] = code_note
            if child[0].tag.split('}')[1]=='UseSelect': # for select statements
                fieldfrom=field_ptr[0].text
                tablename_temp = child[0][0].text.split()
                tablename = ' '.join(tablename_temp[-2:])
                subtable=child[0][1].text
                Mapfrom=child[0][3][0].text
                fieldto=child[0][3][1].text # fieldto- field where the rule is to be written
                globalVariables.dict_op_select[fieldto]=[tablename,subtable,Mapfrom,fieldfrom]
                globalVariables.standard_notes_set.add(fieldto)# set to store fields which should have standard notes populated
            if child[0].tag.split('}')[1] == 'UseSystemVariable':   # for DateTime standard rule
                datatype = ''
                format = ''
                for items in field_ptr[12]:
                    if items.tag.split('}')[1] == 'DataType':
    ...
    globalVariables.dict_1.setdefault(field_ptr[0].text, []).append('')     # store in dict all fields, key-id, value-(seg_name,field_name)
    globalVariables.dict_1.setdefault(field_ptr[0].text, []).append(seg_name)
    globalVariables.dict_1.setdefault(field_ptr[0].text, []).append(field_ptr[1].text)

    field_ct_str = str(field_op_ct)     # field number of that segment
    if len(field_ct_str) == 1:
        field_ct_str = '0' + field_ct_str
    l = [seg_name, field_ct_str]
    globalVariables.dict_ind_field[field_ptr[1].text] = l    #key - fieldName, value - eg(DTM01)
    # print(l)
    globalVariables.dict_opposite_out[''.join(l)]=field_ptr[1].text
    name_field = field_ptr[1].text
    name = ''.join(globalVariables.dict_ind_field[name_field])
    globalVariables.field_set.add(name)
    globalVariables.dict_tag_out[''.join(l)] = [field_tag, field_ct_str]        # to convert field name to format(SegName+SeqNo.)
    globalVariables.dict_type[''.join(l)]=datatype.lower()  # store the dataType of the field
    globalVariables.dict_opposite_group_out[''.join(l)] = group_name

# to process segments(in edi or idoc or csv formats)
def output_seg(group_name,seg_ptr,op_file,name,ct_loop,comp_count,flag,globalVariables):
    field_op_ct=1   # counter to pass to field fn to keep count of the index of the field in the segment(used in naming convention)
    field_tag = ''
    for children in seg_ptr:
        if children.tag.split('}')[1]=="Composite":
        ...
        if children.tag.split('}')[1]=='BlockSig':
            field_tag=children[0].text  # saving the tag_name of the segment
        elif children.tag.split('}')[1] == "Field":
            if flag==0:
                output_field(group_name,children,seg_ptr[1].text,field_tag,field_op_ct,op_file,name,ct_loop,globalVariables)
            else:
                output_field(group_name,children,'Composite'+str(comp_count),field_tag,field_op_ct,op_file,name,ct_loop,globalVariables)
            field_op_ct=field_op_ct+1

# to process groups in mxl file
def output_group(group_ptr,op_file,globalVariables):
    ...
    for children in group_ptr:
        if children.tag.split('}')[1]=="Group": # group could have another group inside
            ...
            name=children[1].text
            ct_loop=children[7].text        # to operate on input side count_loop standard rule
            globalVariables.dict_seg_loop[name]=ct_loop
            comp_count=0
            output_seg(group_name,children,op_file,name,ct_loop,comp_count,0,globalVariables)
        elif children.tag.split('}')[1]=="VarDelimRecord":
...
## fn to put rules in dict if input is xml
def xml_initialize_dict(data_root,fo,globalVariables):
    for child in data_root[3][0]:   # data_root[3] is where details about input start
        if child.tag.split('}')[1]=="XMLElementGroup":
            xml_group_func(child,child[1].text,0,fo,globalVariables)

## fn to put rules in dict if input is edi or idoc or csv
...
## fn to put output extended rules in dict if output is xml
def xml_initialize_output_dict(data_root,op_file,globalVariables):
    for child in data_root[4][0]:       # data_root[4] is where details about output start
        if child.tag.split('}')[1]=="XMLElementGroup":
            xml_output_group_func(child,child[1].text,0,op_file,globalVariables)

## fn to put output extended rules in dict if output is edi or idoc or csv
...
##  get pre-session and post-session rules into the text file with other rules
def initialize_variable_set(data_root,fo,globalVariables):
    ...
    for children in data_root[0]:
        ...
        if children.tag.split('}')[1]=='ExplicitRule':
            ...
            for tags in children:
                print(tags.tag)
                globalVariables.ip_count    # ip_count is to keep count of the input rule number
                if tags.tag!='' and tags.tag.split('}')[1]=='PreSessionRule':
                    if tags.text!='':
                        temp_list = tags.text.split(';')
                        for item in temp_list:
                            globalVariables.ip_count = globalVariables.ip_count + 1
                            globalVariables.dict_field[globalVariables.ip_count] = 'PRE_SESSION'    # dict_field is to store which rule comes at which field
                        fo.write(tags.text)     # write the rule with other rules
                        fo.write('\n')
...
cerror = CaselessKeyword("cerror") + ZeroOrMore(Literal('(')) + Word(nums) + comma + (field_name|str_var|variable) + Optional(comma+(str_var|variable)) + ZeroOrMore(Literal(')'))
...
aton = CaselessKeyword("aton")  + ZeroOrMore(Literal('(')) + (expression|variable_ref|str_var|field_name) +ZeroOrMore(Literal(')'))
...
trim = CaselessKeyword("trim") +ZeroOrMore(Literal('(')) + (expression|variable_ref|str_var|field_name) + Optional(comma + (expression|variable_str|str_var|field_name)) + ZeroOrMore(Literal(')'))
strstr = CaselessKeyword("strstr") + ZeroOrMore(Literal('(')) + (expression|variable_ref|field_name|str_var) + comma +(expression|variable|field_name_up|str_var) + ZeroOrMore(Literal(')'))
ntoa =  CaselessKeyword("ntoa") + ZeroOrMore(Literal('(')) + (expression|variable_ref|str_var|field_name) + comma + (variable_upd|field_name_up) + ZeroOrMore(Literal(')'))
...
mid = CaselessKeyword("mid") + ZeroOrMore(Literal('(')) + (variable_ref|expression|field_name|str_var) + comma + (Word(nums)|expression|variable).setResultsName('START') + comma + (Word(nums)|expression|variable).setResultsName('END') + ZeroOrMore(Literal(')'))
left = CaselessKeyword("left") + ZeroOrMore(Literal('(')) + (expression|variable_ref|field_name|str_var) + comma + ((Word(nums).setResultsName('NUM'))|expression|(str_var)|variable) + ZeroOrMore(Literal(')'))
...
strdate = CaselessKeyword("strdate") + ZeroOrMore(Literal('(')) + (variable_ref|str_var|field_name)+comma+ str_var_format+comma+(field_name_up|variable_upd)+ZeroOrMore(Literal(')'))
...
concat = CaselessKeyword("concat") + ZeroOrMore(Literal('(')) + (field_name_up|variable_upd) +comma+ (field_name|str_var_ref|variable_ref) + comma + (Word(nums).setResultsName('NUM')|variable_num)+ ZeroOrMore(Literal(')'))
delete = CaselessKeyword("delete") + ZeroOrMore(Literal('(')) + (field_name|variable|grp_name_with_arr) + ZeroOrMore(Literal(')'))
years_func = CaselessKeyword("years") + ZeroOrMore(Literal('(')) + (expression|field_name|variable_ref|str_var_ref|Word(nums+'-').setResultsName('REF')) + ZeroOrMore(Literal(')'))
months_func = CaselessKeyword("months") + ZeroOrMore(Literal('(')) + (expression|field_name|variable_ref|str_var_ref|Word(nums+'-').setResultsName('REF')) + ZeroOrMore(Literal(')'))
weeks_func = CaselessKeyword("weeks") + ZeroOrMore(Literal('(')) + (expression|field_name|variable_ref|str_var_ref|Word(nums+'-').setResultsName('REF')) + ZeroOrMore(Literal(')'))
days_func = CaselessKeyword("days") + ZeroOrMore(Literal('(')) + (expression|field_name|variable_ref|str_var_ref|Word(nums+'-').setResultsName('REF')) + ZeroOrMore(Literal(')'))
hours_func = CaselessKeyword("hours") + ZeroOrMore(Literal('(')) + (expression|field_name|variable_ref|str_var_ref|Word(nums+'-').setResultsName('REF')) + ZeroOrMore(Literal(')'))
minutes_func = CaselessKeyword("minutes") + ZeroOrMore(Literal('(')) + (expression|field_name|variable_ref|str_var_ref|Word(nums+'-').setResultsName('REF')) + ZeroOrMore(Literal(')'))
seconds_func = CaselessKeyword("seconds") + ZeroOrMore(Literal('(')) + (expression|field_name|variable_ref|str_var_ref|expression|Word(nums+'-').setResultsName('REF')) + ZeroOrMore(Literal(')'))
length_str = CaselessKeyword("len") + ZeroOrMore(Literal('(')) + (expression|str_var|field_name|variable_ref) + ZeroOrMore(Literal(')'))
...
set_func = CaselessKeyword("set") + (years|months|weeks|days|hours|minutes|seconds) + ZeroOrMore(Literal('(')) + (field_name_up|variable_upd) + comma + (field_name|variable_ref|str_var|Word(nums)) + ZeroOrMore(Literal(')'))
...
expression_without_ref <<  ZeroOrMore('(')+ (func_type|variable|Word(nums+"-")|field_name_wo|str_var)+ZeroOrMore(Literal('(')) + ZeroOrMore((plus|mul|div|left_shift|right_shift|Literal('.')|minus)+ZeroOrMore(Literal('('))+ (func_type|set_scale|variable|Word(nums+"-")|field_name_wo|str_var)+ZeroOrMore(Literal(')'))) + ZeroOrMore(Literal(')'))
# expression << format
...
operator = (Literal('=')|not_equal|less_equal|greater_equal|greater|less|greater_equal|minus).setResultsName('OPERATOR')
...
condition = (ZeroOrMore(Literal('('))+func_type +ZeroOrMore(Literal('('))+ ZeroOrMore(operator +ZeroOrMore(Literal('('))+ (func_type|Word(nums+"-"+'.').setResultsName('UPDATE')|variable_upd|field_name_up|str_var_up)+ZeroOrMore(Literal(')'))) +ZeroOrMore(Literal(')')))|(ZeroOrMore(Literal('('))+(variable_ref|field_name) +ZeroOrMore(Literal('('))+ ZeroOrMore(operator + ZeroOrMore(Literal('('))+(func_type|Word(nums+"-"+'.')|variable_ref|field_name|str_var)+ZeroOrMore(Literal(')')))+ZeroOrMore(Literal(')')))|func_type
...
While_stmt=Forward()
if_stmt=Forward()
select = CaselessKeyword('Select') + (variable|field_name|str_var) + ZeroOrMore(comma+(variable|field_name|str_var)) + CaselessKeyword('into') + (variable_upd|field_name_up) + ZeroOrMore(comma+(variable_upd|field_name_up)) + CaselessKeyword('from') + variable + CaselessKeyword('where')+ conditions + semicol
update = CaselessKeyword('Update') + (variable_upd|field_name_up) + CaselessKeyword('set') + assign_stmt_without_semicolon +  CaselessKeyword('where')+ conditions + semicol
# statement=Forward()
...
# to collect the rule for assign stmt of the if stmt or if some function is present in the rule
def ass_func(tokens,index_start,globalVariables):
    ...
    if tokens[index_start]=='delete' or tokens[index_start]=='sort':
        return '','',i,''   # notes need not be generated for these
    print(store_op)
    print("OPERATORSSSSSSSSSSSSSSSSSSSS")
    rule.append(';')        # appending it to separated parts of right side, so that they can be parsed separately
    ...
    while x<len(rule):
        if rule[x]=='$':
            if 'count' in rule:     # count function could have group name without any field name
                ...
                while rule[x]!=';':
                    x=x+1
                new_rule.append('[')    # replacing all the arr_indexes
                new_rule.append('&')    # finallly the stmt would be "count(grp_name[&]);"
                new_rule.append(']')
                new_rule.append(')')
                new_rule.append(';')
                break
            while x<len(rule) and rule[x]!='#': # to convert $grp_name#func_name to segName+seqNo.
                x=x+1
            x=x+1
            ...
            x=x+1
            continue
        if rule[x]=='#':     # if only fn name is present
            x=x+1
            if rule[x] in globalVariables.dict_ind_field :   # converting fnName to segName+seqNo.
                t=''.join(globalVariables.dict_ind_field[rule[x]])
            else:
                t=rule[x]
            new_rule.append(t)
    ...
    final_stmt=''
    print(assign_stmts)
    for ind,stmts in enumerate(assign_stmts):   # for each of the split parts
        print("HI")
        print(stmts)
        ...
        if stmts.find('!')>=0:
            stmts = stmts.replace('!', '')
            exist = 1   # to be used later
        func_tokens = start.parseString(stmts)
        temporary_rule=''
        print("******************S")
        if func_tokens:
            for token in func_tokens[0]:    # func_tokens[0] would have the tokens
                ...
                if token == 'Update' or token == 'select':
                    break
        if not temporary_rule:  # if no stmt is got from fn_for_fn_new
        ...
        if ind==0:
            ...
            if len(store_op)!=0 and op_in<len(store_op):
                if store_op[op_in]=='<<':
                        final_stmt=final_stmt + 'add' + temporary_rule  # operator specific req.
                elif store_op[op_in]=='>>':
                    final_stmt=final_stmt + 'subtract' + temporary_rule
                else:
                    final_stmt=final_stmt + store_op[op_in] + temporary_rule    # concatenating the stored operators at their positions
                op_in=op_in+1
            else:
                final_stmt=temporary_rule
    ...
    if 'REF' in rule_tokens[1]:
        y=rule_tokens[1]['REF']
    return x,y,i,final_stmt  # x- update var, y-ref_variables list, i-index where this ass_rule ends, final_stmt-the rule string formed

### for processing the functions used in normal assignment statements
### similar as function_for_functions_new
...
# to process direct assignment stmts without any functions in them
def assign_func(index,tokens,ip_or_op,dict_token,globalVariables):  # index is the rule number
    rule_str = ''.join(tokens)
    rule_str=rule_str.replace('=',' = ')    # because later splitting on space
    rule_tokens = statement.parseString(rule_str)
    rule_tokens = repr(rule_tokens)
    rule_tokens = eval(rule_tokens)
    ref=""
    ...
    if ip_or_op==0:
        field=globalVariables.dict_field[index] # field is the field name on which this rule has been written
    else:
        field=globalVariables.dict_op_field[index]

    t=field
    if field in globalVariables.dict_ind_field:
        t=''.join(globalVariables.dict_ind_field[field])    # changing it to segName+SeqNo.
        flag=1
    print(t)
    ...
    for ind,item in enumerate(ref):
        if ind!=0:
            right_side+=' + '
        if item in globalVariables.dict_ind_field:  # i.e. item is a field
            right_side+= ''.join(globalVariables.dict_ind_field[item])
        else:
            right_side+= item   # i.e item is some variable
    ...
    l=[]
    if upd in globalVariables.variable_set :    # if a variable is being updated
        l=[ref,'',rule]
    elif ip_or_op==1:
        print(ref)
        if len(ref)==1 and ref[0] not in globalVariables.variable_set:  # lef(ref)==1 means only one thing on right side..probably string
...
def if_util(tokens,index,globalVariables):
    ...
    while tokens[i]!='then':
        ...
        i=i+1
    new_cond=[]     # making a new list out of cond list
    x=0
    print("COND")
    print(cond)
    while x<len(cond) :
        if cond[x]=='$':    # changing the field name into "segName + index no."
            if 'count' in cond:
            ...
            x=x+1
            continue
        if cond[x]=='#':    # incase there is no group name...only field name is there
            x=x+1
            if cond[x] in globalVariables.dict_ind_field:
                t=''.join(globalVariables.dict_ind_field[cond[x]])
                new_cond.append(t)
            ...
            x=x+1
            continue
        if cond[x] in globalVariables.dict_ind_field:   # if the field name is written without hashtag
            t=''.join(globalVariables.dict_ind_field[cond[x]])
            new_cond.append(t)
        else:
            new_cond.append(cond[x])
...
def while_func_complex(tokens,index,stack,dict_token,globalVariables):
    # print(tokens)
    i=index
    ...
    if tokens[i].lower()=='begin':
        temp_stack.append('begin')
        tokens[i]=''
        i=i+1
        while i<len(tokens):
            if tokens[i]=='begin':
                temp_stack.append('begin')
            elif tokens[i].lower() == 'end':
                temp_stack.pop()
                if len(temp_stack) ==0:
                    tokens[i]=''
                    break
    ...
    # final_ind=eval_sent(tokens,index,stack,dict_token,globalVariables)
    # print(dict_token)
    # exit()
    return index
...
def eval_sent(tokens, index, stack,dict_token,globalVariables):
    ...
    if i<len(tokens) and (tokens[i].lower()=='continue' or tokens[i]=='break'):
        return i+2  # skipping continue and semicol
    if i<len(tokens) and tokens[i]=='if':
        i=if_func(tokens,i,stack,dict_token,globalVariables)    # for nested if... i is the position in the rule till where it has been processed
        return i
    elif i<len(tokens) and tokens[i] == 'while':
        i=while_func_complex(tokens,i,stack,dict_token,globalVariables)
        # print(tokens)
        # exit()
        return i
    else:
        update,ref,i,rule=ass_func(tokens,i,globalVariables)    # for assign stmt

        # print(tokens[i])
        temp_list=[]
        temp_str=""
        for list_temp in stack:
            temp_list.extend(list_temp[0])  # combining the reference variables in all the conditions
            if temp_str:
                temp_str += (" and "+list_temp[1])  # combine all the condition strings
            else:
                temp_str=list_temp[1]   # for the first condition
        temp_list.extend(ref)
        if update in dict_token:
            dict_token[update].append([temp_list,temp_str,rule])    # temp_str-combination of all conditions, rule-final stmt
        else:
            temp=[]
...
def if_func(tokens,index, stack,dict_token,globalVariables):    #index- char no. from which the stmt starts
    print(tokens)
    i=index+1
    cond_tokens, cond_str,i=if_util(tokens,i,globalVariables)   # get the condition of if-stmt
    temp=[cond_tokens,cond_str]
    print("this is the list")
    print(temp)
    stack.append(temp)  # stack is there for nested if stmts- to combine conditions
    temp_list=[]
    if i<len(tokens) and tokens[i]=='begin':
        i=i+1
        while i<len(tokens) and tokens[i]!='end':
            i=eval_sent(tokens,i,stack,dict_token,globalVariables)  # for each of the stmts inside the if condition
        temp_list=stack.pop()       # pop out that condition from the stack
    else:
        i=eval_sent(tokens,i,stack,dict_token,globalVariables)  # in case of only single stmt inside the if condition
        temp_list=stack.pop()
    ...
    while i<len(tokens) and tokens[i]=='else if':
        print("ok else if")
        i=i+1
        c_t,c_s,i=if_util(tokens,i,globalVariables)     # get the condition
        t=[c_t,c_s]
        stack.append(t)

        if i < len(tokens) and tokens[i] == 'begin':    # similar process as 'if'
            i = i + 1
            while i < len(tokens) and tokens[i] != 'end':   # for multiple stmts inside the elif condn
                i = eval_sent(tokens, i, stack,dict_token,globalVariables)
            stack.pop()
        else:
            i = eval_sent(tokens, i, stack,dict_token,globalVariables)  # in case there is only one stmt inside the elif
            stack.pop()
    ...
    if i<len(tokens) and tokens[i]=='else':
        ...
        # print(temp_str)
        # new_str='!('+temp_str+')'
        new_str=temp_str.replace('=',' != ')    # negate the prev if stmt
        new_list=[temp_list[0],new_str]
        stack.append(new_list)
        i=i+1
        if tokens[i] == 'begin':
            i = i + 1
            while tokens[i] != 'end':   # same conditions as if and elif
                i=eval_sent(tokens, i, stack,dict_token,globalVariables)
            stack.pop()
        else:
...
def decl_func(tokens,ip_or_op,globalVariables): # ip_or_op=0 if i/p , =1 if o/p
    rule = ' '.join(tokens)
    rule_tokens = statement.parseString(rule)
    rule_tokens = repr(rule_tokens)
    rule_tokens = eval(rule_tokens)
    ...
    print(var_type)
    list_of_variables=rule_tokens[0][0][1]['VARIABLES'] # for stmts like int a,b,c; list_of_var will have a,b,c
...
### to remove java statements from the textfile
def handle_java(line_list,arr_java,null_flag,globalVariables):  # arr_java is the set having all variables related to java stmts
    for ind, line in enumerate(line_list):  # null_flag- if a rule continues in multiple lines(is 1 till a semicol is found)
        if not line:
            continue
        if (null_flag == 1):
            line_list[ind] = ''
            if ';' in line:
                null_flag = 0   # if end of ongoing java stmt is found
                continue
        obj_split = line.split(' ', 1)  # checking if it is "obj a;" kind of stmt
        if obj_split[0].lower() == 'object':
            for item in obj_split[1].split():
                item = item.replace(';', '')
                arr_java.append(item.strip())   # add the obj variable to the arr_java set
        if obj_split[0].strip() in arr_java:
            line_list[ind]=''
            continue
        obj_split_sec = line.split('.',1)   # checking for a.fn() type of statements
        if obj_split_sec[0].strip() in arr_java:
            line_list[ind]=''
            continue
        flag = 0
        # print("helo")
        split_list = line.split('=', 1)     # checking for a=b.fn() type of stmts
        if len(split_list) == 1:
            continue
        ...
        for item in arr_java:
            # print(item)
            if item.strip() in right_part:  # checking for occurrence of any item in arr_java in stmt
                print(item)
                field_name=left_part.split('#')     # getting the field name
                if len(field_name)>1:
        ...
        if flag == 1:
            if ';' not in line:
                null_flag=1
            line_list[ind] = left_part + " = java;"     # converting the line in format a=java;
            # print("final_line")
            # print(line_list[ind])

    block_code = '\n'.join(line_list)   # join all the processed stmts
...
def while_func(token_list,dict_token,ip_or_op,globalVariables):
    # print(token_list)
    i=0
    while(token_list[i].lower()!="do"):
        token_list[i]=''
        i=i+1
    token_list[i]=''
    i=i+1
    if token_list[i].lower()=='begin':
        token_list[i]=''
        token_list[len(token_list)-1]=''
    token_list = list(filter(None, token_list))
    # print(token_list)
    token_str=' '.join(token_list)
    result=start.parseString(token_str)
    # print(result)
    make_dictionary(result,dict_token,ip_or_op,globalVariables)
    # print(dict_token)
    # exit()
...
def make_dictionary(result,dict_token,ip_or_op,globalVariables):
    for index, result_token in enumerate(result):   # index - rule no. , result_token- rule
        print(result_token)
        flag = 0    # to check if the rule has been processed
        # if  'while' in  (name.lower() for name in result_token):
        #     continue
        if result_token[0].lower() == 'while':
            # exit()
            while_func(result_token,dict_token,ip_or_op,globalVariables)
            continue
        if result_token[0] == 'if':
            stack = []
            i = if_func(result_token, 0, stack,dict_token,globalVariables)   # i is the char no. till the rule has been read
            flag = 1
            continue
        ...
        for rule_token in result_token: # if the rule is function assignment
            if rule_token in func_list: # func_list has all the function names
                print("SOWMITHHHHHHH")
                upd, ref, n, rule = ass_func(result_token, 0,globalVariables)   # upd-variable/fn_name being updated, ref-list of referred variables
                if upd in dict_token:                               # rule-assignment rule
                    dict_token[upd].append([ref, "", rule])  # "" because there is no condition
                else:
                    t = []
                    t.append([ref, "", rule])
                    dict_token[upd] = t
        ...
        if flag == 0:
            for rule_ind, rule_token in enumerate(result_token):    # iterating the rule
                if rule_token == '=':   # checking if it's a direct assignment stmt
                    if result_token[rule_ind+1]=='0' or (result_token[rule_ind+1]=='"' and result_token[rule_ind+2]=='"'):  # i.e if it is a null declaration
                        # print(result_token)
                        # exit()
                    ...
                    print("SDDDDDD")
                    # print(index)
                    assign_func(index, result_token,ip_or_op,dict_token,globalVariables)    # else, it's a normal assign stmt
                    flag = 2
                    break
        ...
        if flag == 0:
            ...
            print(result_token)
            decl_func(result_token,ip_or_op,globalVariables)    # for declaration statement
        # print(globalVariables.dict_token)
    ...
    for list_temp in temp_lists:
        ...
        for var_var in x :
            ...
            if (var_var in globalVariables.dict_token ) and (inp_op==0):
                temp_list_note.extend(find_in_dict_token(var_var,note,temp_dict,inp_op,var,list_recur,globalVariables))
            if ((var_var in globalVariables.dict_token) or (var_var in globalVariables.dict_op_token)) and (inp_op==1):
                temp_list_note.extend(find_in_dict_token(var_var,note,temp_dict,inp_op,var,list_recur,globalVariables))
        temp_note=""
...
def replace_add(st,inp_op,globalVariables):
    ...
    for value in list_of_values:
        value=value.strip()
        if value in globalVariables.field_set:  # if the argument is a field name
            var_type=''
            if inp_op==0:
        ...
        if value.isdigit(): # a numberic is one of the arguments
            flag=1
            print("Yes")
            break
        ll=value.split()
        for it in ll:
            if it=='integer' or it=='product' or it=='divide':  # if these keywords are coming
    ...
    if flag==1:
        ...
        return ' + '.join(list_of_values)   # i.e.the arguments are strings
    ...
    for ind,var in enumerate(temp_list):
        ...
        for i,tk in enumerate(ll):
            if tk.strip() in temp_dict:
                if temp_dict[tk]:
                    res=temp_dict[tk][0]    #one var could have multiple values assigned at diff places...extracting the first one
...
#             elif ((var_list[0].strip() in globalVariables.variable_set) and (len(var_list[0].split())==1)):
#                 print("or here")
#
...
def format_ind_note(note,globalVariables):
    ...
    for  index,condition in enumerate(condition_list_new):
        ...
        if len(var_list)==2:
            ...
            elif ((var_list[0].strip() in globalVariables.variable_set) and (len(var_list[0].split())==1)) or (temp_string.strip() in globalVariables.variable_set): # if it is "var=a" type of condition
                print("or here")
...
### makes the notes by combining the variables referred
def final_note_for_field(field_ptr,temp_id,inp_op,globalVariables):  # temp_id-- for which field or variable the note is being created
    if inp_op==0:   # i.e. if creating for input side rules
        temp_list=globalVariables.dict_op[temp_id]      # dict_op[temp_id]-list with field_name to which temp_id is linked
    ...
    list_note = find_in_dict_token(temp_list[0], note,temp_dict,inp_op,'',list_recur,globalVariables)
                                    # temp_list[0]- name of the field
    print(list_note)                # list_note would return a list of lists...each list corresponds to each rule
    print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
    print(temp_dict)
    if not list_note:
        return
    ...
    for list_temp in list_note:
        ...
        for line in lines:
            ...
            if line:
                ...
                ll=line.split()
                sql=0
                if len(ll)>=2 and ll[0]=='and' and ll[1].lower()=='if': # i.e. the rule is an if-stmt
                    ll.pop(0)       # remove the 'and'
                ...
                while l!=0 :
                    if(ll[l]=='='):
                        equal_ind=l
                    if (ll[l]=='and'):
                        ll[l]='then'        # insert a 'then'
                        break
                    l=l-1
                if equal_ind!=0:
                    del ll[l+1:equal_ind+1]     # remove the var which is being updated..e.g( if a=b then c=d) remove 'c=',insert " map"
                    if l==0:
                        ll[0]=" Map  "
                    else:
                        ll.insert(l+1," Map  ")
                    ...
                    print(dest)
                    dc={}
                    dd=get_dest(dest,temp_dict,dc,globalVariables)  #to format the right side of the if-stmt
                    print(dd)       # dc is the dictionary returned ... it has the value assigned to variables on right side
                    print("thois is dd")
                    print(dc)
                    print("printing the dictionary created")
                    dest_temp[0]=dest_temp[0].replace('!=""'," is not empty ")
                    split_list=re.split('\s',dest_temp[0])      #'\s'-space
                    print(split_list)
                    print("list split")
                    for split_ind, split_item in enumerate(split_list):
                        if split_item in dc:
                            split_list[split_ind]=dc[split_item]        # replace the value of each var on left side by using dict-dc
                    split_result=' '.join(split_list)
                    fla=1
                    print(dd)
                    while fla==1:       # some string formatting
                        print("HI")
                        fla=0
                        if dd.find('" ')>=0:
                            dd=dd.replace('" ','"')
                    ...
                    dd=dd.replace('""','" "')
                    print(dd)
                    if dd[0]=='"':      # i.e. if right side starts with a string
                        dd_t=replace_mul(dd,globalVariables)        # to make arithmetic string
                        dd_t = dd_t.replace("divide  \" by \"", "\"/\"")    # string manipulation
...
def output_field_note(field_ptr,inp_op,globalVariables):
    if field_ptr[3].text=='0':
        return
    if inp_op==0:       # that is for input side rules
        temp_id = field_ptr[0].text
        final_note=''
        if temp_id in globalVariables.dict_op_ct:   # there exists a constant standard rule
            print(temp_id)
            const_val=int(globalVariables.dict_op_ct[temp_id][0])   # dict_op_ct-has the values that need to be hardcoded
            if const_val in globalVariables.dict_constant:
                globalVariables.f.write("FIELD: "+ globalVariables.dict_useless[temp_id] +"\n")
                if globalVariables.dict_op_ct[temp_id][1]:      # if there is condition in the st rules
                    final_note+="If " + globalVariables.dict_1[globalVariables.dict_op_ct[temp_id][1]][2] + " exists then "
        ...
        select_stmt=''
        if temp_id in globalVariables.dict_op_select:   # for 'Use select' standard rule
            ...
            select_stmt="Populate "+v[2]+" from CODELIST = " +globalVariables.dict_constant[int(v[1])][2] +" into "+fieldto+" where "+v[0]+" = "+fieldfrom+'\n'
            globalVariables.dict_notes[field_ptr[1].text][1]=select_stmt

        date_note=''
        if temp_id in globalVariables.dict_op_date: # for 'DateTime' standard rule
        ...
        if temp_id in globalVariables.dict_op:       # for links to input rules
        ...
        if temp_id in globalVariables.dict_op_token:
            if temp_id in globalVariables.dict_opposite_name_id:
                ...
                globalVariables.extended_notes_set.add(field_id)        # to track that there should be a rule populated(used at the end)            output_note=final_note_for_field(field_ptr,temp_id,1,globalVariables)
            if output_note[0]=='#':
...
def output_group_note(group_ptr,globalVariables):
    for children in group_ptr:
        ...
        elif children.tag.split('}')[1]=="Segment":     # when o/p is edi
            output_seg_note_link(children,globalVariables)
            output_seg_note_op(children,globalVariables)
        elif children.tag.split('}')[1]=="PosRecord":   # when o/p is idoc
            output_seg_note_link(children,globalVariables)
            output_seg_note_op(children,globalVariables)
        elif children.tag.split('}')[1]=="VarDelimRecord":  # when o/p is csv
            output_seg_note_link(children,globalVariables)
            output_seg_note_op(children,globalVariables)

# for input side and standard rules
def xml_output_rec_note_link(rec_ptr,globalVariables):
    for children in rec_ptr:
        if children.tag.split('}')[1]=="Field":
            output_field_note(children,0,globalVariables)

# this one is to create notes for the output side extended rules
def xml_output_rec_note_op(rec_ptr,globalVariables):
    for children in rec_ptr:
        if children.tag.split('}')[1]=="Field":
            output_field_note(children,1,globalVariables)   # 1 is for output
...
def xml_output_group_note(group_ptr,globalVariables):
    for children in group_ptr:
        if children.tag.split('}')[1]=="XMLElementGroup":
            ...
            xml_output_rec_note_link(children,globalVariables)   # for input side and standard rules
            xml_output_rec_note_op(children,globalVariables)    # for output side rules
...
def xml_make_notes(data_root,globalVariables):
    for child in data_root[4][0]:       # data_root[4] - start of the output tag
        if child.tag.split('}')[1]=="XMLElementGroup":
            xml_output_group_note(child,globalVariables)

def initialize_const_Map(data_root,globalVariables):    # data_root is the file_pointer of the mxl file
    sowmith=0
    # if data_root.text=='UseConstant':
    print(len(data_root))
    # exit()
    # if len(data_root)>=7:
    for real_child in data_root:        # constant_map tag has the mapping from index to the value to be hardcoded
...
## this function is called to get the rule for extended rules
def check_for_note(list_from_dict,i,globalVariables):   # i is the index of the dict_notes which has the rule... check in output_field_note_combine
    note=''
    ...
    for line in output_note_list:
        ...
        if (line.split()[0].lower() == 'populate' or line.split()[0].lower() == 'hardcode') and len(line.split('+'))==1:
            ...
            break
        # note=''
        if i!=3 and flag==0:        # i.e. there is a standard rule at the field
            note = list_from_dict[i]+"\n"
            flag=1
        if line.split()[0].lower() == 'if':
            note += line+"\n"       # concatenate the standard rule and extended rule
        elif line.split()[0].lower() == 'select':
            note += line + "\n"     # concatenate the standard rule and extended rule
            split_list = line.split()
            end_ptr = len(split_list) - 1
            while split_list[end_ptr] != '=':
                end_ptr = end_ptr - 1   # for each of the fields referred in the select stmt, get their notes, and concatenate
            ...
            note += line +"\n"      # concatenate the standard rule and the extended rule
        else:
            note += "\n"+ line
    if note:
        return note
...
def change_format(note,i_f,o_f,globalVariables):    # called in the end to change the format of the field names in the final notes formed
    ...
    for line_ind,line in enumerate(line_list):
            ...
            if token in globalVariables.dict_opposite:
                ...
                if field in globalVariables.dict_name_id:               ## that is it's input field
                    if i_f=='IDOC' or i_f=='XML' or i_f=='CSV':
                        ...
                        # print(globalVariables.dict_1)
                        # exit()
                        field_name_list=globalVariables.dict_1[field_id]     # field_name_list[1]-segmentName, field_name_list[2]-sequenceNumber
                        token_list[ind]=field_name_list[1]+"/"+field_name_list[2]
                    if i_f=='EDI':
                        grp_name = globalVariables.dict_opposite_group[token]
                        if grp_name!=globalVariables.inp_grp_name:  # inp_grp_name- the main group of the input side
                ...
                if field in globalVariables.dict_opposite_name_id:      ## that is it's output field
                    ...
                    if o_f=='EDI':
                        ...
                        if grp_name!=globalVariables.out_grp_name:  # out_grp_name-the main group of the output side
                            # token_list[ind] = grp_name+"/"+''.join(globalVariables.dict_tag_out[token])
        ...
        line_list[line_ind]=' '.join(token_list)    # i.e. finally change the field name
    print('\n'.join(line_list))
    return '\n'.join(line_list)



def output_field_note_combine(field_ptr,i_f,o_f,pseudo_pointer,globalVariables):    # field_ptr-where the notes are to be written
    # print("output_field_note_combine")                                            # incase of xml, this is actually group ptr
    # if field_ptr[3].text=='0':
    #     return
    if field_ptr[3].text == '0':
        if o_f=="XML":
            globalVariables.link_notes_set.discard(pseudo_pointer[0].text)       # pseudo_ptr is given only for xml files
            globalVariables.standard_notes_set.discard(pseudo_pointer[0].text)  # reqd only in case when the fields are under attribute section,
            globalVariables.extended_notes_set.discard(pseudo_pointer[0].text)  # the name of the field and group is not same....
        else:
            globalVariables.link_notes_set.discard(field_ptr[0].text)
            globalVariables.standard_notes_set.discard(field_ptr[0].text)   # all these sets are to keep track of which fields should have had notes
            globalVariables.extended_notes_set.discard(field_ptr[0].text)   # and which dont...(to generate log file)
        return
    ...
    temp_id = field_ptr[1].text  # i.e. field_name
    ...
    if not list_from_dict[0] and not list_from_dict[1] and not list_from_dict[2] and not list_from_dict[3] and not list_from_dict[4] and not list_from_dict[5]:
        return  # i.e. field has no note to be populated
    print(list_from_dict)
    print("DS1")
    if list_from_dict[0]:   # field has a 'Use constant' rule
        if o_f=="XML":
            ...
            globalVariables.link_notes_set.discard(field_ptr[0].text)
        if list_from_dict[3]:       # field also has extended rule
            note=''                 # so it would overwrite the hardcode rule
            note=check_for_note(list_from_dict,0,globalVariables)   # get the note
            note=change_format(note,i_f,o_f,globalVariables)         # change the field name syntax depending on i/p and o/p format
            print("CH")
            print(note)
            # exit()
            ...
            if note_note[0].lower()=='hardcode':
                ...
                note_note[len(note_note)-1]=note_note[len(note_note)-1]+'"' # putting the constant value in quotes
            note=' '.join(note_note)
            field_ptr[5].text=note       # write the note at the field... field_ptr[5] is the note tag
            if o_f=="XML":
    ...
    if list_from_dict[4]:       # field has accumulator standard rule
        if o_f=="XML":
            globalVariables.link_notes_set.discard(pseudo_pointer[0].text)
            globalVariables.standard_notes_set.discard(pseudo_pointer[0].text)
        else:
        ...
        if list_from_dict[3]:
            note=''
            note=check_for_note(list_from_dict,4,globalVariables)   # 4 is the index of the dict_notes
            note_note=note.split()
            if note_note[0].lower()=='hardcode':
                note_note[1]='"'+note_note[1]
    ...
    if list_from_dict[5]:   # field has dateTime standard rule
        if o_f=="XML":
            globalVariables.link_notes_set.discard(pseudo_pointer[0].text)
            globalVariables.standard_notes_set.discard(pseudo_pointer[0].text)
        else:
    ...
    if list_from_dict[1]:       # field has a 'Select' standard rule
        if o_f=="XML":
            globalVariables.link_notes_set.discard(pseudo_pointer[0].text)
            globalVariables.standard_notes_set.discard(pseudo_pointer[0].text)
        else:
        ...
        while split_list[end_ptr] != '=':
            end_ptr = end_ptr - 1
        split_list_res = split_list[end_ptr+1:] # get the last part of the rule after the '='
        print(split_list_res)
        fields = ' '.join(split_list_res)
        field_list = fields.split('+')
        print(field_list)
        flag=0
        for item in field_list:     # for each of the referred fields after '=', get the notes
        ...
        if list_from_dict[3]:       # if there is also an extended rule
            note=check_for_note(list_from_dict,1,globalVariables)       # no need to write the standard rule(extended rule overwrites the standard rule)
            note_note=note.split()
            if note_note[0].lower()=='hardcode':
                note_note[1]='"'+note_note[1]
                note_note[len(note_note)-1]=note_note[len(note_note)-1]+'"'
    ...
    if list_from_dict[2]:   # field has a link rule
        if o_f=="XML":
            globalVariables.link_notes_set.discard(pseudo_pointer[0].text)
        else:
            globalVariables.link_notes_set.discard(field_ptr[0].text)
...
def xml_output_rec_note_combine(rec_ptr,i_f,o_f,group_ptr,globalVariables):
    ...
    for children in rec_ptr:
        ...
        if children.tag.split('}')[1] == "Field":
            if rec_type.lower() == 'pcdata':            # grp_ptr is given because in xml the notes are to be populated at the
                output_field_note_combine(group_ptr, i_f, o_f, children,globalVariables)    # group pointers
            else:
                output_field_note_combine(children, i_f, o_f, children,globalVariables)     # if the fields are under the attribute tag
                                                                                            # in this case notes are to be populated at fields

def xml_output_particle_note_combine(particle_ptr,i_f,o_f,globalVariables):     #i_f-input format, o_f-output format
    # print("particle")
    for children in particle_ptr:
        if children.tag.split('}')[1]=="XMLElementGroup":
...
## start of formatting and populating the final notes- when o/p is xml
...
def sort_java_arr(globalVariables):
    ...
    globalVariables.link_notes_set.difference_update(globalVariables.standard_notes_set)    # if a field has link+standard rule, it should be removed from link_set
...
## all the notes are populated at the relevant fields here
def write_func(data_root,etree,raw_data,filenameUploaded,outputPath,globalVariables):
    ...
    raw_data.write(finalNotesPath, encoding='utf-8', xml_declaration=True)   # writing into mxl
...
def make_log(lf,globalVariables):
    ...
    for item in globalVariables.link_notes_set:
        field_list=globalVariables.dict_1[item]  # o/p fields linked to input side fields
        field=field_list[1]+'/'+field_list[2]
