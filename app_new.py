...
def authorizeCouch():
    user = "admin"
    password = "admin123"
    return couchdb.Server("http://%s:%s@9.199.145.193:5984/" % (user, password))
...
@app.route('/downloadMAP',methods = ['GET','POST'])
def downloadMAP():
    ...
    if request.method =='GET':
        ...
        for document in db:
            if(db[document]['email']==session['email']):
                ...
                try:
                    team=db[document]['team'] 
                    session['team']=team
                except Exception as e:
                    print("Select a team to update")
                    team="None"
                    session['team']=team  
                break 
        if(idForUser==""):
            # dictSaved['_id'] = session['username']
            ...
            try:
                ...
                for document in db:
                    if (db[document]['email'] == session['email']):
                        idForUser = db[document]['_id']
                        dictSaved = db[document]
                        dictSaved['usage'] = db[document]['usage']
                        print("usage retrieved successfully")

                        # else append usage list
                else:
                    # dictSaved['_id'] = session['username']
                    dictSaved['usage'].append({'fileName': filenameUploaded, 'time': timeaccessed, 'status':status})
                    print("Usage updated sucessfully")
                    dictSaved['team'] = select

                # save to db
                db.save(dictSaved)
                #return downloadMAP(ipfile)
                return render_template('file_upload.html', user=session['username'], team=session['team'], filename = ipfile)
            except Exception as e:
                print(e)
                dictSaved = {}
                for document in db:
                ...
                # save to db
                #db.save(dictSaved)
                return render_template('file_upload.html',user=session['username'],team=session['team'],info = "Sorry!! This Map is currently not supported.")
            #return downloadMAP(notesMadeFile)
    # if('logged_in' not in session or 'username' not in session):
    #     return redirect(url_for('index'))
    return render_template('file_upload.html',user=session['username'],team=session['team'])
